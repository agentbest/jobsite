...
> | **1-2. 中途／新卒を見分ける列（`messages.site` / `profiles.site`）** | **✅ 実行済み**（2026-09-11） |
> | 2. 運営アカウント作成＋ staff 登録 | **✅ 完了**（`r_matsuoka+scout@agent-best.net` / staff 登録済み） |
> | 3. Apps Script の貼り直し＋既存デプロイの更新 | **✅ 完了**（**バージョン6**・2026/09/11 2:33・URLは変わらず） |
> | 4. `messages_notify` ウェブフック | **✅ 作成済み**（profiles_notify と同じURL・同じ合言葉） |
> | 4-補. Redirect URLs に `http://127.0.0.1:8787/**` | **✅ 追加済み**（手元で会員ログインを試すため） |
> | 5. サイトの push | **✅ 完了**（2026-09-11・jobsite `4c86953` / shinsotsu `d154893`） |
>
> **疎通確認済み**：`messages` に1件入れて通知メールが1秒で届くことを確認し、その行は削除しました
> （Supabase → ウェブフック → Apps Script → Gmail が一本につながっています）。
>
> **2026-09-11の確認**：デプロイ済みのウェブアプリに直接POSTして、`site` による出し分けを確かめました。
> `site='shinsotsu'` → 求人・マイページとも `shinsotsu.agent-best.net`／`site='jobsite'` → `jobs.agent-best.net`／
> 会員からの返信は担当者あてに「（新卒サイトの会員です）」／`site` を埋めただけの `profiles` の更新は
> `skipped: site backfill` でメールを出さない、の4点。管理画面も両サイトの求人を読み（中途5,732＋新卒40）、
> サイト違いの求人を添えたままでは送れないことを確認済み。
>
> **会員としてログインして `profiles.site` に `jobsite` が入ることも確認済み**（マジックリンク）。
>
> ⚠ このとき **「会員情報が更新されました」が2通飛びました**。ログイン直後は auth のイベントが重なって
> `onSignedIn` が2〜3回走り、3回とも `site` が空のまま読んでから書くため、2回目以降が
> 「`site` が既に埋まっている状態での更新」になって最初の判定をすり抜けていたのが原因です。
> `template.html` に `siteRemembered` フラグを足し、Apps Script 側も
> **`site` があって `updated_at`・`resume_at`・`cv_at` が1つも動いていない更新は通知しない**ようにしました。
> 本物のプロフィール更新は今までどおり通知されることも確認済みです。
>
> | アカウント | 役割 |
> |---|---|
