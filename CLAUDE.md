...
`rebuild.js` の **`hiClassOnly()`** が、職種の大分類が **その他／小売・飲食／交通・運輸** で、かつ年収の上限（無ければ下限）が **400万円未満** の求人を落とす
（`LOW_GROUPS` / `LOW_SALARY_CAP`）。工場のライン作業・フォークリフト・店舗スタッフなど、媒体から一括で入ってきた求人が対象。初回は130件（ヨコタエンタープライズ103件）。

- **年収が未記載の求人は判定できないので載せる**（同じ大分類で174件）。件数はビルド時に出る。
