...
問い合わせが入ると Airtable のオートメーション `wflaOu5LOuWTDwia1` が Slack **#notification_siteoubo** へ通知する。**応募導線を消したのでこのチャンネルは他に流れるものがない**（元は応募通知先）。⚠ **APIから作ったオートメーションは下書きのままなので、Airtableの画面でオンにしないと動かない。**
