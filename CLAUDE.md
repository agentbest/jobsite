...
### ハイクラス訴求に合わない求人は載せない（2026-09-22・松岡さん決定）

`rebuild.js` の **`hiClassOnly()`** が、職種の大分類が **その他／小売・飲食／交通・運輸** で、かつ年収の上限（無ければ下限）が **450万円未満または未記載** の求人を落とす
（`LOW_GROUPS` / `LOW_SALARY_CAP`）。工場のライン作業・フォークリフト・店舗スタッフなど、媒体から一括で入ってきた求人が対象。
最初は「400万円未満・未記載は載せる」で130件だったが、405万円のフォークリフト求人とキッチンスタッフ（年収未記載）が残ったので同日に今の基準にした（365件）。

- 3分類の外（機械・営業など）に付いた製造派遣（日研トータルソーシング）・介護系（SMS）はこの基準では残る。職種名の語で落とす案は誤爆が出るので採らなかった。
- Airtable は触らない。掲載を止める判断はビルド側（`midCareerOnly()` と同じ考え方）。
- 基準を変えたら `job/`・企業ページの募集中件数も一緒に変わる。翌朝の自動更新で反映される。
- ⚠ 新卒サイト（shinsotsu）にはこの関数は無い（新卒に年収の下限基準は馴染まないため）。
...
- 管理画面を `file://` で開かない。`スカウト管理画面.bat`（`admin/server.js`・127.0.0.1:8787）で開く。
  **2026-09-22〜: この端末ではタスクスケジューラ `jobsite-scout-admin` がログオン時に `admin/start-hidden.vbs` 経由でサーバーを黒い窓なしで常駐させる**ので、
  ブラウザのブックマーク `http://127.0.0.1:8787/admin/` だけで開ける。.bat は他端末用・保険として残す。止まっていたら .bat か `Start-ScheduledTask jobsite-scout-admin`。
...
### 新卒サイト（shinsotsu）と共用している

⚠ **Supabase プロジェクト `jobsite-tokyo` は2サイトで共有している**（別プロジェクトではない）。
SQL・管理画面・ウェブフック・Apps Script はそのまま1本で両サイトを見ている。
会員側UIは 2026-09-05 に shinsotsu の `template.html` へ移植済み。

**どちらのサイトの人／求人かは列で持つ**（2026-09-11・agentbest/backlog #20）。

| 列 | 入れる人 | 値 |
|---|---|---|
| `profiles.site` | 各サイトの `template.html` の `rememberSite()`（初回ログインで1度だけ） | `jobsite` / `shinsotsu` |
| `messages.site` | 管理画面が送信時に会員のサイトを入れる | 同上（既定 `jobsite`） |

- ⚠ **`template.html` 冒頭の `SITE_KEY` を書き換えないこと。** 中途は `"jobsite"`、新卒は `"shinsotsu"`。
  間違えると、相手のサイトのマイページ・求人へリンクした通知メールを送ることになる
- ⚠ **`rememberSite()` は既にある値を上書きしない。** 両方のサイトを使う人がいるので「最初に登録したサイト」を残す。
  `updated_at` も触らない（触ると「会員情報が更新されました」の通知が飛ぶ）。
  Apps Script 側でも、site を埋めただけの更新は `skipped: site backfill` で捨てている
- **空の `site` は中途扱い**（新卒サイトの公開は2026-09-06で、それ以前の会員は全員中途）。
  ⚠ **埋め直すバックフィルSQLを流さないこと**（更新した件数だけ通知メールが飛ぶ）
- 管理画面は**両サイトの `jobs.json` を読む**（新卒は `https://shinsotsu.agent-best.net/data/jobs.json` を直接。
  GitHub Pages が `Access-Control-Allow-Origin: *` を返すのでクローン不要）。
  求人と会員のサイトが食い違ったままでは**送信できない**（注意書きを出して止める）
- ⚠ **`template.html` のメッセージ関連は2サイトで鏡写し。片方だけ直さないこと**（ロゴ・従業員数・左サイドバーと同じ）
...
- ⚠ **親の名前と子の名前を重ねないこと。** `state.sel` は親も子も同じ Set に入れているので、
  同じ名前があると親子の区別がつかない
- 中分類→大分類の対応（`IND_GROUP`）は **2026-09-22 から Airtable の業界マスタをそのまま埋める**（`fetch-companies.js` → `data/industry-master.json` → `rebuild.js` の `attachIndustryGroups()` → `__IND_GROUP__`）。手書きの対応表は廃止。マスタに中分類を足せば次のビルドで自動で出る。
  ⚠ 新卒サイト（shinsotsu）は手書きの `IND_GROUP` のまま（求人40件なので影響が小さい）。
- **求人の業界は企業DBの業界で上書きする**（`rebuild.js` の `attachIndustry()`。2026-09-22〜）。求人票テーブルの「業界カテゴリ」リンクは取り込み時の写しで、企業側を直しても追随しないため。企業DBに無い会社・企業側が空の会社だけ求人側の値を使う。
- 業界マスタの大分類「その他」は 2026-09-22 に「サービス・外食」に改名した（アミューズメント／レストラン・フードサービス／専門・その他サービス）。企業ページの業界ページ名もこれに揃う。
...
### 5. 勤務地の都道府県は Airtable の列が正（2026-09-22〜）

求人DB（求人票）に **「勤務地（都道府県）」`flddnoD5qZtuAISOO`（複数選択・47都道府県＋全国＋海外）** を新設した。
`fetch-jobs.js` が `prefs` として取り込み、`rebuild.js` の **`prefsOfJob()`** が「列があればそれ、空なら原文から推測（`areasOf`）」で `areas` を決める。

- 初回は原文から機械的に5,415件を投入した（県名あり5,088／市区町村・略称284／全国43）。残り363件は空のまま＝サイトは従来どおり推測する。
  変更内容は `企業DB拡充_20260920\勤務地都道府県_初回投入_20260922.tsv`。
- **エリアで出したい求人は Airtable のこの列を埋める。** 本文の書き方に関係なく確実に出る。
- 「全国」は絞り込みの値としてそのまま出る（ブロックに属さないので左サイドバーでは「その他」の下）。`/area/` の静的ページは作らない。
- ⚠ 会社の本社所在地では代用しない。テクノプロ・デザイン社（本社=六本木／勤務地=取引先構内で全国）のような求人が東京都で出てしまうため。
- ⚠ 新卒サイト（shinsotsu）はこの列を読んでいない（従来どおり推測のまま）。

### 6. 勤務地が書かれていない求人を減らした（`areasOf()`）
